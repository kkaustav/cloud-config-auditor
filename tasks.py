...
def grade_easy(findings, severity, recommendations, config_patch):
    ...
    bd["patch_bonus"] = 0.10 if config_patch and "22" not in json.dumps(config_patch).lower() else 0.0
...
def grade_medium(findings, severity, recommendations, config_patch):
    ...
    wildcard_expanded = issues["wildcard_princ"] + ["principal", "public read", "unrestricted", "overly permissive"]
    ...
    bd["rec_quality"]         = 0.05 if _contains(rec_text, ["enable versioning", "block public", "kms", "least privilege", "mfa delete"]) else 0.0
    bd["severity_bonus"]      = 0.05 if "high" in sev_text or "critical" in sev_text else 0.0
    bd["patch_bonus"]         = 0.05 if config_patch and any(k in json.dumps(config_patch).lower() for k in ["aes256","kms"]) else 0.0
    score = round(max(0.01, min(0.99, sum(bd.values()))), 2)
    return score, bd
...
def grade_hard(findings, severity, recommendations, config_patch):
    ...
    bd["patch_bonus"]      = 0.08 if config_patch and '"flowlogsenabled": true' in json.dumps(config_patch).lower() else 0.0
    score = round(max(0.01, min(0.99, sum(bd.values()))), 2)
    return score, bd
