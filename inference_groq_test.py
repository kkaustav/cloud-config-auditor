...
def run_task(task):
    ...
    obs = requests.post(f"{BASE_URL}/reset?task={task}", timeout=30).json().get("observation", "")
    messages = [
    ...
    for step in range(1, 6):
        ...
        try:
            step_resp = requests.post(f"{BASE_URL}/step", json={"action": action_str}, timeout=30).json()
        except Exception as e:
        ...
        if env_feedback:
            ...
            messages.append({"role": "user", "content": f"Environment feedback: {env_feedback}\nRefine your audit JSON."})
